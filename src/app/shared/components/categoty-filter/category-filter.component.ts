...
export class CategoryFilterComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.queryParams.subscribe(params => {
      ...
      // После обновления страницы, нужно чтобы блок с фильтрами был открыт с выбранными параметрами фильтрации
      if (this.type) {
        if (this.type === 'height') {
        ...
        if (
          ...
          this.categoryWithTypes.types.some(type => {
            return this.activeParams.types.find((item: string): boolean => type.url === item);
          })
        ) {
          this.open = true;
        } else {
  ...
  //Функция сварачивания блоков фильтрации
  toggle(): void {
    this.open = !this.open
  }
