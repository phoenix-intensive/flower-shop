...
export class OrderComponent implements OnInit {
  ...
  constructor(private cartService: CartService, private userService: UserService, private router: Router, private _snackBar: MatSnackBar,
              private fb: FormBuilder, private dialog: MatDialog, private orderService: OrderService, private authService: AuthService) {

    //Функция обязательной валидации в зависимости от способа доставки
  ...
  ngOnInit(): void {
    ...
    //Проверяем залогинен ли полз или нет, иначе нет смысла делать запрос на получения инф из ЛК
    //Далее запрашиваем получения данных о пользователе из ЛК, чтобы инф подставилась в инпуты формы из ЛК
  ...
  //Функция подсчета общей суммы товаров и кол-ва товара
  calculateTotal(): void {
    this.totalAmount = 0;
    this.totalCount = 0;
    if (this.cart) {
  ...
  updateDeliveryTypeValidation(): void {
    //Делаем обязательную валидацию в зависимости от способа доставки
    if (this.deliveryType == DeliveryType.delivery) {
      ...
      //Чтобы не отправлять лишние данные на сервер мы очищаем форму при выборе способа доставки "Самовывоз"
    ...
    //Для того чтобы метод валидации при переключении метода доставки обновился и преминился используется метод updateValueAndValidity()
  ...
  createOrder(): void {
    if (this.orderForm.valid && this.orderForm.value.firstName
      ...
      this.orderService.createOrder(paramsObject)
        .subscribe({
          next: (data: OrderType | DefaultResponseType): void => {
            ...
            //После подтверждения заказа кол-во заказов в корзине измениться на ноль!!!
      ...
      //markAllAsTouched() - подсветит те поля которые пользователю нужно заполнить!
